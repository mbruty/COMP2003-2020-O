...
const uploadFileMutation = gql`
  mutation UploadFile($file: Upload!, $crop: CropInput!, $auth: AuthInput!, $foodID: Int!) {
...
const DragNDrop: React.FC<Props> = (props) => {
  const [uploading, setUploading] = React.useState<{ uploadClicked: boolean, uploading: boolean }>({ uploadClicked: false, uploading: false });
  const [file, setFile] = React.useState();
  const [previewUrl, setPreviuewUrl] = React.useState<string>("");
  const [cropState, setCropState] = React.useState(defaultCrop);
  const [cropData, setCropData] = React.useState<{ x: number, y: number, width: number, height: number } | undefined>();
  ...
  const onCropComplete = (croppedArea: any, croppedAreaPixels: any) => {
    setCropData({ x: croppedAreaPixels.x, y: croppedAreaPixels.y, width: croppedAreaPixels.width, height: croppedAreaPixels.height })
  }
  ...
  return (
    <>
      ...
      {file &&
        <>
          <div style={{ position: "relative" }} className="drag-n-drop-container">
            <Cropper
              image={previewUrl}
              crop={cropState.crop}
              zoom={cropState.zoom}
          ...
          <div style={{ display: "grid", placeItems: "center", width: "10q0%" }}>
            {!uploading.uploadClicked &&
              <Button onClick={onSave} variant="contained" color="primary" style={{ marginBottom: 5, width: "100%" }}>Upload</Button>
            }
            {uploading.uploadClicked &&
              <LoadingBtn
            ...
            <Button onClick={() => { setFile(undefined); destroyUrl(); setCropState(defaultCrop); }} variant="contained" style={{ width: "100%" }} >Cancel</Button>
          </div>
        </>
      }
    </>
